...
function renderFlags() {
    ...
    challenges.forEach(ch => {
        ...
        if (solved) {
            ...
            badge.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> COMPLETED';
...
async function submitFlag(challengeId) {
    ...
    // Already solved locally — no need to submit
    if (solvedFlags[challengeId]) return;
    ...
    const btn = input.parentElement?.querySelector('.flag-submit');
    ...
    try {
        ...
        addLog('Network error — check connection and retry', 'error');
        return;
    }
    ...
    if (res.status === 200) {
        if (res.data.correct) {
            ...
            if (res.data.already_solved) {
                // Recover from prior 500 — sync score from server
                myScore = res.data.total_score;
                myFlagCount = res.data.total_flags;
                addLog('Flag was already accepted — synced.', 'success');
            } else {
        ...
        addLog('Server error — retrying...', 'error');
        // Auto-retry once after 2 seconds on 500
        setTimeout(() => submitFlag(challengeId), 2000);
    } else if (res.status === 429) {
        addLog('Too fast — wait a moment and retry', 'error');
    } else {
        addLog(res.data?.error || 'Flag submission failed', 'error');
    }
}
...
async function exitAfterKick() {
    // End session on server (already kicked, but properly close it)
...
async function endEvent() {
    ...
    // Also end session to decrement player count
    await window.ctfAPI.request('/end-session', 'POST', {
        session_id: sessionId,
    }, token);
    ...
    // If result screen wasn't shown yet (e.g. slot < 2 min), show it now
    if (!resultShown) {
        resultShown = true;
        showFinalResult();
    }
...
async function confirmExit() {
    if (!eventEnded) {
        eventEnded = true;
        // End session on server — this decrements player count
        await window.ctfAPI.request('/end-session', 'POST', {
            session_id: sessionId,
        }, token);
    }
