...
// ==================== WARNINGS ====================
// Monitoring runs for real: the camera watches, the mic watches, lost focus and
// devtools are noticed, and every one of those raises a visible warning that
// counts up on screen. What AUTO_KICK controls is only the consequence.
//
//   false — warnings are shown and counted, and that is where it ends. Nobody
//           is removed by the app, and warnings are not reported to the server
//           either, since the server removes a player at three of them.
//   true  — the original behaviour: three warnings and the player is out.
//
// An admin can still remove someone by hand from WordPress in both cases.
const AUTO_KICK = false;
// ==================================================
...
// ===== Float Windows: drag + resize =====
(function initFloatWindows() {
    const EDGE = 7;          // how close to a border counts as "grabbing" it
    const MIN_W = 240;
    const MIN_H = 130;

    let topZ = 51;
    let drag = null;         // { win, dx, dy }
    let resize = null;       // { win, dir, x, y, l, t, w, h }

    // The windows are laid out in CSS with a mix of left, right, bottom and —
    // for the Challenges window — a translateX(-50%) centring trick. Dragging
    // or resizing needs a plain left/top/width/height box instead. Clearing the
    // transform is what stops Challenges jumping half its width on first drag.
    function pin(win) {
    ...
    function edgeAt(e, win) {
        const r = win.getBoundingClientRect();
        let d = '';
        if (e.clientY - r.top < EDGE) d += 'n';
        else if (r.bottom - e.clientY < EDGE) d += 's';
        if (e.clientX - r.left < EDGE) d += 'w';
        else if (r.right - e.clientX < EDGE) d += 'e';
        return d;
    }
    ...
    // Hovering near a border shows the resize cursor.
    document.addEventListener('pointermove', e => {
        if (drag || resize) return;
        const win = e.target.closest('.float-window');
        if (!win) return;
        win.style.cursor = cursorFor(edgeAt(e, win)) || '';
    });
    ...
    document.addEventListener('pointerdown', e => {
        ...
        const dir = edgeAt(e, win);
        if (dir) {
            pin(win);
            const r = win.getBoundingClientRect();
            resize = { win: win, dir: dir, x: e.clientX, y: e.clientY, l: r.left, t: r.top, w: r.width, h: r.height };
        ...
        pin(win);
        const r = win.getBoundingClientRect();
        drag = { win: win, dx: e.clientX - r.left, dy: e.clientY - r.top };
        win.setPointerCapture(e.pointerId);
        e.preventDefault();
    });
    ...
    document.addEventListener('pointermove', e => {
        if (resize) {
            const s = resize, d = s.dir;
            const mx = e.clientX - s.x, my = e.clientY - s.y;
            let l = s.l, t = s.t, w = s.w, h = s.h;

            if (d.includes('e')) w = Math.max(MIN_W, s.w + mx);
            if (d.includes('w')) { w = Math.max(MIN_W, s.w - mx); l = s.l + s.w - w; }
            if (d.includes('s')) h = Math.max(MIN_H, s.h + my);
            if (d.includes('n')) { h = Math.max(MIN_H, s.h - my); t = s.t + s.h - h; }

            // Stay on screen
            l = Math.max(0, Math.min(l, window.innerWidth - MIN_W));
            t = Math.max(0, Math.min(t, window.innerHeight - MIN_H));
            w = Math.min(w, window.innerWidth - l);
            h = Math.min(h, window.innerHeight - t);

            s.win.style.left = l + 'px';
            s.win.style.top = t + 'px';
        ...
        if (drag) {
            ...
            const maxY = window.innerHeight - drag.win.offsetHeight;
            drag.win.style.left = Math.max(0, Math.min(e.clientX - drag.dx, maxX)) + 'px';
            drag.win.style.top = Math.max(0, Math.min(e.clientY - drag.dy, maxY)) + 'px';
        }
    });
...
// Kiosk lockdown is decided per event by the Proctoring setting in WordPress,
// and applied by applyProctoring() once the challenge payload arrives. It stays
// false until then — the init code that reads it runs after that call.
...
(async function init() {
    ...
    if (slotRes.status === 200) {
        const me = slotRes.data.players.find(p => String(p.user_id) === String(user.id));
    ...
    if (chRes.status === 200) {
        ...
        // Proctoring is set per event in WordPress. With it off the app drops
        // out of kiosk so players can alt-tab to their own tools — that is the
        // whole point of challenges that hand out a download link. An older
        // server that doesn't send the field is treated as locked down.
        applyProctoring(chRes.data.proctoring);
    ...
    if (isKioskMode) {
        ...
        // Skipped when any challenge carries a download link, since that link
        // is copied on purpose and wiping it 5 seconds later would look like
        // the copy button is broken.
        const hasLinks = challenges.some(c => c.resource_url);
        if (!hasLinks) {
            setInterval(() => {
                try { navigator.clipboard.writeText(''); } catch (e) { }
            }, 5000);
        } else {
            addLog('Clipboard clearing paused — this event hands out download links', 'warn');
        }
    }
    ...
    // Start camera + mic monitoring after cinematic intro finishes (7s)
    // Skip if user skipped the device test — no camera/mic violations
    const cameraSkipped = sessionStorage.getItem('ctf_camera_skipped') === 'true';
    if (!cameraSkipped) {
        // Start mic monitoring (volume only — no recording)
        startMicMonitoring();
        setTimeout(() => startCameraMonitoring(), 7000);
    } else {
        addLog('Camera & mic monitoring skipped (device test was skipped)', 'info');
    }
})();

// ===== Security Check =====
// There used to be a runSecurityCheck() here that blocked entry outright on a
// second monitor or a screen recorder. Nothing ever called it, and it would
// have locked out ordinary players anyway — a projector reads as a second
// monitor, and NVIDIA and Xbox Game Bar background processes read as screen
// recorders on a normal Windows laptop. The periodic check below covers the
// same ground as a warning instead of a wall.
...
function startEventTimer() {
    ...
    function tick() {
        ...
        // Show results screen at 2 minutes remaining
        if (diff <= 120000 && !resultShown) {
            resultShown = true;
            showFinalResult();
        }
...
function showFinalResult() {
    ...
    const totalPlayers = lbBody ? lbBody.children.length : 1;
    const rankEl = document.getElementById('myRank');
    const myRankNum = rankEl ? parseInt(rankEl.textContent.replace('#', ''), 10) || totalPlayers : totalPlayers;

    let rankColor = '#e0e0e0';
    if (myRankNum === 1) rankColor = '#FFD700';
    else if (myRankNum === 2) rankColor = '#C0C0C0';
    else if (myRankNum === 3) rankColor = '#CD7F32';
    else if (myRankNum <= Math.ceil(totalPlayers * 0.3)) rankColor = '#00e87b';

    const overlay = document.getElementById('resultOverlay');
    const box = overlay.querySelector('.result-box');
    box.innerHTML = `
        <div style="text-align:center;padding:20px 10px;">
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:3px;color:var(--text-muted);margin-bottom:20px;">Event Complete</div>
            <div style="font-size:64px;font-weight:800;color:${rankColor};font-family:'JetBrains Mono',monospace;line-height:1;">#${myRankNum}</div>
            <div style="font-size:13px;color:var(--text-secondary);margin-top:8px;">out of ${totalPlayers} players</div>
            <div style="font-size:11px;color:var(--text-muted);margin-top:20px;">${myScore} pts \u00b7 ${myFlagCount} flags solved</div>
            <div class="result-code" id="resultCode" style="display:none;margin-top:16px;padding:10px 16px;background:rgba(0,232,123,0.08);border:1px solid rgba(0,232,123,0.2);border-radius:6px;font-family:'JetBrains Mono',monospace;font-size:13px;color:#00e87b;"></div>
            <button class="btn" style="margin-top:24px;padding:10px 28px;" onclick="exitEvent()">Exit</button>
        </div>
    `;
...
// The app never fetches the file. It shows the link and copies it, and the
// player pastes it into their own browser — which is why a 500 MB forensics
// image costs this app and the server nothing.
function resourceRowHtml(ch) {
    if (!ch.resource_url) return '';
    const label = ch.resource_label || 'Download file';
    // The URL is passed by challenge id, not written into an attribute.
    // escapeHtml() escapes text, not attribute values — it leaves quotes alone —
    // so a URL containing one could otherwise break out and add its own
    // attribute. Ids are integers, which is safe either way.
    const id = parseInt(ch.id) || 0;
    return `
        <div class="flag-resource">
            <div class="flag-resource-text">
                <span class="flag-resource-label">${escapeHtml(label)}</span>
                <span class="flag-resource-url">${escapeHtml(ch.resource_url)}</span>
            </div>
            <button class="flag-resource-copy" onclick="copyResource(${id}, this)">Copy link</button>
        </div>`;
}

async function copyResource(challengeId, btn) {
    const ch = challenges.find(c => String(c.id) === String(challengeId));
    const url = ch && ch.resource_url;
    if (!url) return;

    const ok = await window.ctfAPI.copyText(url);
    const original = btn.textContent;
    btn.textContent = ok ? 'Copied' : 'Copy failed';
    btn.classList.add(ok ? 'copied' : 'failed');

    if (ok) {
        addLog('Challenge link copied — paste it in your browser to download', 'info');
    } else {
        showErrorToUser('Could not copy the link. Select it and copy by hand.', 'error');
    }
...
// Reading and answering are two different jobs, so they get two windows.
// Brief holds everything the player reads — the story, the download link, the
// hint. Submit holds nothing but the inputs, which keeps it small enough to sit
// beside whatever they're working in.

const TICK_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5"/></svg>';
const RING_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="9"/></svg>';

function emptyState(message) {
    return `<div class="empty-state">${escapeHtml(message)}</div>`;
}

// ===== Brief: story, links, hints =====
function renderBrief() {
    const body = document.getElementById('briefBody');
    if (!body) return;

    if (challenges.length === 0) {
        body.innerHTML = emptyState('Waiting for challenges to load.');
        return;
    }
    ...
    if (eventDescription) {
        html += `<div class="brief-intro">${escapeHtml(eventDescription)}</div>`;
    }

    challenges.forEach(ch => {
        const solved = solvedFlags[ch.id];
        html += `
            <div class="brief-item${solved ? ' solved' : ''}">
                <div class="brief-head">
                    <span class="brief-num">${parseInt(ch.step_number) || 0}</span>
                    <span class="brief-title">${escapeHtml(ch.title || '')}</span>
                    <span class="brief-pts">${parseInt(ch.points) || 0} pts</span>
                    <span class="brief-state">${solved ? TICK_SVG : RING_SVG}</span>
                </div>
                ${ch.description ? `<div class="brief-desc">${escapeHtml(ch.description)}</div>` : ''}
                ${resourceRowHtml(ch)}
                ${ch.hint ? `<div class="brief-hint"><span>Hint</span>${escapeHtml(ch.hint)}</div>` : ''}
            </div>`;
    });
...
function renderFlags() {
    ...
    challenges.forEach(ch => {
        ...
        const id = parseInt(ch.id) || 0;
        html += `
            <div class="sub-row${solved ? ' solved' : ''}">
                <span class="sub-num">${parseInt(ch.step_number) || 0}</span>
                <span class="sub-title">${escapeHtml(ch.title || '')}</span>
                ${solved
                ? `<span class="sub-solved">${TICK_SVG} Solved</span>`
                : `<input type="text" class="flag-input" id="flag_${id}" placeholder="flag{...}" autocomplete="off" spellcheck="false">
                       <button class="flag-submit-btn" onclick="submitFlag(${id})">Submit</button>`}
            </div>`;
    });
...
// Both windows show solved state, so they always redraw together.
...
async function submitFlag(challengeId) {
    ...
    let res = await window.ctfAPI.request('/submit-flag', 'POST', {
        session_id: sessionId,
        challenge_id: challengeId,
        answer: answer,
    }, token);

    // Auto-retry once on 500 (handles first-submit race condition where session isn't fully synced)
    if (res.status === 500) {
        await new Promise(r => setTimeout(r, 1500));
        res = await window.ctfAPI.request('/submit-flag', 'POST', {
            session_id: sessionId,
            challenge_id: challengeId,
            answer: answer,
        }, token);
    ...
    if (res.status === 200) {
        if (res.data.correct) {
            ...
            updateScoreDisplay();
            renderChallenges();
            addLog(`You completed step ${challenges.find(c => c.id === challengeId)?.step_number} — +${res.data.points} pts`, 'success');
            showPointsToast(res.data.points);
        } else {
        ...
        const errMsg = res.data?.error || 'Flag submission failed (status ' + res.status + ')';
        addLog(errMsg, 'error');
        showErrorToUser(errMsg, 'error');
    }
}
...
function showPointsToast(points) {
    ...
    const text = document.getElementById('pointsToastText');
    if (!toast || !text) return;
    text.textContent = `+${points} pts`;
    toast.classList.add('show');
    if (showPointsToast._timer) clearTimeout(showPointsToast._timer);
    showPointsToast._timer = setTimeout(() => {
...
// Fake Instagram / WhatsApp popups used to appear here mid-event as bait.
// Removed: a paying player being taunted with counterfeit notifications is not
// a premium experience, and it made the app look like a mock-up.

// ===== Proctoring =====
// WordPress decides per event whether this run is locked down. Off is the
// default and matches how the app has always behaved; on turns the window into
// a kiosk so a real competition can be run from the same build.
function applyProctoring(flag) {
    isKioskMode = flag === 1 || flag === '1' || flag === true;

    if (!isKioskMode) {
        addLog('Proctoring off — window mode, you can switch to other apps', 'info');
        return;
    }

    window.ctfAPI.enableKiosk();
    startAntiDevTools();
    addLog('Proctoring on — secure kiosk locked', 'info');
}
...
// Camera status line. Written through one helper so every state gets the same
// coloured dot and the same wording, instead of each call site inventing its
// own glyph.
function setCamStatus(el, text, state) {
    if (!el) return;
    const colours = { ok: '#00e87b', warn: '#f59e0b', bad: '#ef4444' };
    const colour = colours[state] || colours.ok;
    el.style.color = colour;
    el.innerHTML = '<span class="cam-dot" style="background:' + colour + '"></span>' + escapeHtml(text);
}
...
async function acquireCamera(videoEl, statusEl, canvasEl) {
    ...
    try {
        ...
        if (videoTrack) {
            ...
            videoTrack.addEventListener('unmute', () => {
                if (cameraState === 'lost' || cameraState === 'recovering') {
                    ...
                    cameraState = 'active';
                    faceAbsentCount = 0;
                    setCamStatus(statusEl, 'MONITORING', 'ok');
                }
            });
        }
        ...
        if (cameraState === 'recovering') {
            ...
            setCamStatus(statusEl, 'RECONNECTING', 'warn');
        } else if (cameraState === 'off') {
            // First-time failure — critical
            cameraState = 'lost';

            // Without auto-kick there is no removal countdown and no triple
            // warning — say the camera is off and carry on.
            if (!AUTO_KICK) {
                addLog('Camera unavailable — monitoring paused', 'warn');
                showErrorToUser('Camera not available — continuing without it.', 'camera');
                setCamStatus(statusEl, 'NO CAMERA', 'warn');
                return;
            }

            showErrorToUser('CRITICAL: Camera access denied! You will be removed in 30 seconds.', 'kick');
            setCamStatus(statusEl, 'NO CAMERA', 'bad');
            setTimeout(() => {
...
function handleCameraLost(videoEl, statusEl, canvasEl) {
    ...
    cameraRecoveryTimer = setInterval(async () => {
        ...
        setCamStatus(statusEl, 'RECONNECTING (' + recoveryAttempts + ')', 'warn');
        ...
        try {
            ...
            if (recoveryAttempts >= 12) {
                ...
                clearInterval(cameraRecoveryTimer);
                cameraRecoveryTimer = null;
                setCamStatus(statusEl, 'CAMERA LOST', 'bad');
                showErrorToUser('Camera could not be reconnected.', 'error');
...
function startFaceDetectionLoop(videoEl, statusEl, canvasEl) {
    ...
    faceCheckInterval = setInterval(async () => {
        ...
        if (faceFound) {
            ...
            setCamStatus(statusEl, 'FACE DETECTED', 'ok');
        } else {
            faceAbsentCount++;
            if (faceAbsentCount <= 2) {
                setCamStatus(statusEl, 'SCANNING', 'warn');
            } else {
                setCamStatus(statusEl, 'NO FACE (' + faceAbsentCount + ')', 'bad');
                showErrorToUser('Face not detected — look at the camera. (' + faceAbsentCount + ')', 'camera');

                // Snapshots are evidence for a removal, so they are only worth
                // uploading when a removal can actually happen.
                if (faceAbsentCount === 3 && AUTO_KICK) {
        ...
        // 4 consecutive misses (20 seconds) becomes a formal warning
        const now = Date.now();
...
function handleKeyBlock(e) {
    ...
    if (blocked.some(Boolean)) {
        // Swallow the key, but don't hold it against the player. Reaching for
        // F5 or Ctrl+S is muscle memory, not cheating, and three reflexes
        // should not cost someone their event.
        e.preventDefault();
        e.stopPropagation();
        if (AUTO_KICK) reportWarning('blocked_key:' + e.key);
    }
}
...
function handleInternetLost() {
    ...
    // Venue wifi drops on its own and is not cheating, so this only removes
    // anyone when auto-kick is deliberately switched on.
    if (AUTO_KICK && internetWarnings >= 3) {
...
async function reportWarning(reason) {
    ...
    // Every violation in the app funnels through here, so this is the one place
    // that decides whether a warning can end someone's event. With AUTO_KICK off
    // the player still sees the count climb — it just never removes them, and it
    // is kept off the server, which would otherwise remove them at three.
    if (!AUTO_KICK) {
        const shown = Math.min(warnings, 3);
        // The toast fires every time; the full-screen overlay only on the first
        // and the third. A face check that keeps missing would otherwise throw a
        // modal across the screen every thirty seconds for the rest of the event.
        showErrorToUser(`Security Warning ${shown}/3: ${reason.replace(/_/g, ' ')}`, 'error');
        if (warnings === 1 || warnings === 3) showWarningScreen(shown);
        else document.getElementById('myWarnings').textContent = shown + '/3';
        addLog('Warning ' + shown + '/3 — ' + reason.replace(/_/g, ' '), 'warn');
        return;
    }
...
function showWarningScreen(count) {
    // Clamp: warnings keep counting past three when auto-kick is off, and "5/3"
    // on screen reads like a bug.
    const shown = Math.min(count, 3);
    document.getElementById('warningCount').textContent = shown + '/3';
    document.getElementById('warningOverlay').classList.add('show');
    document.getElementById('myWarnings').textContent = shown + '/3';
}
...
async function endEvent() {
    ...
    // If result screen wasn't shown yet, show it now
    if (!resultShown) {
        resultShown = true;
        showFinalResult();
    }
...
function navigateBrowser() {
    ...
    if (frame) {
        ...
        // Validate URL to prevent javascript:, data:, or other dangerous protocols
        try {
...
// Started by applyProctoring() rather than at load, because whether this event
// is proctored isn't known until the challenge payload comes back.
let antiDevToolsStarted = false;

function startAntiDevTools() {
    if (antiDevToolsStarted) return;
    antiDevToolsStarted = true;
