...
// ==================== CSP NOTE ====================
// A per-load nonce used to be generated here and stamped onto every <script>
// tag, under the heading "nonce-based CSP". The CSP below never referenced it —
// script-src carried 'unsafe-inline' — so it protected nothing while looking
// like it did.
//
// A nonce also cannot help yet: it only covers <script> elements, and the UI is
// wired with inline onclick attributes throughout, which need 'unsafe-inline'
// regardless. Moving those to addEventListener is what would make a nonce
// meaningful. Until then this is stated plainly instead of implied.
...
// Admin generates config.json from the WordPress plugin, places it in electron-app/, then builds the EXE
// Players never see any config screen.

function getBundledConfigPath() {
    if (app.isPackaged) {
        // Config is inside ASAR (not extraResources) to avoid NSIS $ escaping issues
        return path.join(__dirname, 'config.json');
    }
    return path.join(__dirname, 'config.json');
}

// Must match CTF_ADMIN_PW_SALT / CTF_ADMIN_PW_ITERATIONS in the WordPress plugin.
const ADMIN_PW_SALT = 'hacklayer-ctf-admin-v2';
const ADMIN_PW_ITERATIONS = 210000;
...
// Called before app exit/close to properly end the player's session
async function endSessionOnServer() {
    if (!currentSessionId || !currentToken) return;
    try {
        const timestamp = Date.now().toString();
        const sig = signRequest('/end-session', 'POST', timestamp);
        ...
        // Best-effort — server will auto-cleanup stale sessions via cron
    }
}
...
function detectSuspiciousProcesses() {
    return new Promise((resolve) => {
        exec('tasklist /FO CSV /NH', { encoding: 'utf8', timeout: 5000, windowsHide: true }, (err, stdout) => {
            if (err) return resolve([]);
            const running = stdout.toLowerCase();
            resolve(suspiciousList.filter(p => running.includes(p)));
        });
    });
}

// Detect VM environment (using reg query — much faster than wmic)
function detectVM() {
    return new Promise((resolve) => {
        exec('reg query "HKLM\\HARDWARE\\DESCRIPTION\\System\\BIOS" /v SystemManufacturer', {
            encoding: 'utf8', timeout: 3000, windowsHide: true
        }, (err, stdout) => {
            if (err) return resolve(false);
            const output = stdout.toLowerCase();
            const vmIndicators = ['virtualbox', 'vmware', 'qemu', 'hyper-v', 'xen', 'parallels', 'virtual machine'];
            resolve(vmIndicators.some(v => output.includes(v)));
        });
    });
}
...
function detectSystemProxy() {
    return new Promise((resolve) => {
        exec('reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" /v ProxyEnable', {
            encoding: 'utf8', timeout: 3000, windowsHide: true
        }, (err, stdout) => {
            if (err) return resolve(false);
...
app.whenReady().then(() => {
    ...
    if (!loadConfig()) {
        ...
        dialog.showErrorBox('Configuration Missing', 'config.json not found.\n\nAdmin: in WordPress go to CTF Manager, then Settings, then Download EXE Config. Place it in the electron-app folder and rebuild the EXE.');
        app.quit();
        return;
    }
    ...
    if (!ADMIN_PASSWORD_HASH || !/^[0-9a-f]{64}$/.test(ADMIN_PASSWORD_HASH)) {
        const { dialog } = require('electron');
        dialog.showErrorBox('Invalid Configuration', 'Admin password not configured.\n\nSet the admin password in WordPress under CTF Manager, then Settings. Download a new config.json and rebuild the EXE.');
        app.quit();
        return;
    }
    ...
    protocol.handle('app', async (request) => {
        ...
        // Electron normalizes app:///src/file to app://src/file — 'src' becomes hostname
        // Reconstruct full path: hostname (if any) + pathname
        let filePath = decodeURIComponent((url.hostname ? '/' + url.hostname : '') + url.pathname);
        ...
        if (ext === '.html') {
            ...
            // Remove meta CSP tags — CSP is enforced directly via Response headers below
            content = content.replace(/<meta[^>]*Content-Security-Policy[^>]*>/gi, '');

            // Build CSP with nonce — applied directly on Response for reliable enforcement
            // in packaged builds where onHeadersReceived may not fire for custom protocols
            const apiOrigin = API_BASE ? API_BASE.replace(/\/wp-json\/ctf\/v1$/, '').replace(/\/+$/, '') : '';
            const csp = [
                ...
                `connect-src 'self' app: ${apiOrigin}`,
                "media-src 'self' app: blob: mediastream:",
    ...
    // CSP for app:// is handled directly in protocol.handle() Response headers.
    // This interceptor only applies CSP to external HTTP/HTTPS requests (e.g., webview frames).
    session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
        // Skip app:// protocol — CSP is already embedded in the Response by protocol handler
        if (details.url && details.url.startsWith('app://')) {
        ...
        const csp = [
            ...
            "font-src 'self' app: https://fonts.gstatic.com",
            "img-src 'self' app: data: blob: https:",
            `connect-src 'self' app: ${API_BASE.replace('/wp-json/ctf/v1', '')}`,
            "media-src 'self' app: blob: mediastream:",
            "frame-src 'self' https:",
            "object-src 'none'",
            "base-uri 'self' app:",
...
ipcMain.handle('security-check', async () => {
    ...
    // Run process scan, VM check, and proxy check in parallel (non-blocking)
    const [procs, isVM, hasProxy] = await Promise.all([
    ...
    if (isVM) threats.push('virtual_machine');
    if (hasProxy) threats.push('system_proxy');
...
// Verify admin password — scrypt hash comparison with brute force protection
...
ipcMain.handle('verify-admin-password', (event, password) => {
    ...
    // PBKDF2-SHA256, matching CTF_ADMIN_PW_SALT and CTF_ADMIN_PW_ITERATIONS in
    // the WordPress plugin. This used to be scrypt here while the plugin stored
    // a bare sha256, so the two never matched and the admin exit password could
    // never verify — which, with kiosk mode on, meant no way out of the app.
    // Both sides must derive identically; do not change one without the other.
    const derived = crypto.pbkdf2Sync(password, ADMIN_PW_SALT, ADMIN_PW_ITERATIONS, 32, 'sha256').toString('hex');
    // Timing-safe comparison to prevent side-channel attacks
    const derivedBuf = Buffer.from(derived, 'utf8');
    const expectedBuf = Buffer.from(ADMIN_PASSWORD_HASH, 'utf8');
    const valid = derivedBuf.length === expectedBuf.length && crypto.timingSafeEqual(derivedBuf, expectedBuf);
    if (!valid) {
...
ipcMain.handle('navigate', (event, page) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
        // The whole flow: login -> category -> camera-test -> secure-env,
        // with profile hanging off category. Nothing else exists.
        const allowedPages = [
            'login.html',
            'category.html',
            'camera-test.html',
...
// Clear session data — notify server first
...
// Exit app — end session on server before quitting
...
// Open external URL in system browser (strict protocol + domain validation)
ipcMain.handle('open-external', (event, url) => {
    try {
        if (typeof url !== 'string' || url.length > 2048) return;
        const parsed = new URL(url);
        // Only allow http/https protocols (block javascript:, file:, data:, etc.)
        if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') return;
        // Block localhost/internal network to prevent SSRF
        const hostname = parsed.hostname.toLowerCase();
        if (hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '::1' ||
            hostname.endsWith('.local') || hostname.startsWith('192.168.') ||
            hostname.startsWith('10.') || hostname.startsWith('172.')) return;
        require('electron').shell.openExternal(url);
    } catch (e) { /* invalid URL, ignore */ }
});

// Copy text to the clipboard. Only used for challenge download links, so the
// same http/https rule as open-external applies — nothing else should be able
// to push arbitrary content onto the user's clipboard from the renderer.
...
app.on('will-quit', async (event) => {
    // End session on server before fully quitting
    if (currentSessionId && currentToken) {
        event.preventDefault();
        await endSessionOnServer();
        globalShortcut.unregisterAll();
